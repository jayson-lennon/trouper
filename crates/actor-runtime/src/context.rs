...
pub trait RuntimeView: Send + Sync {
    ...
    fn lookup(&self, path: &Path) -> Option<crate::registry::EndpointInfo>;
...
impl CtxCore<'_> {
    ...
    pub fn lookup(&self, path: &Path) -> Option<crate::registry::EndpointInfo> {
...
mod tests {
    ...
    struct FakeView {
        paths: Vec<(Path, ActorKind)>,
        handlers: Vec<(SchemaId, Path)>,
        now: Timestamp,
    }
    ...
    fn reply_targets_the_requester_only_when_reply_to_exists() {
        ...
        let reply_to = Address::Path(Path::new("client"));
    ...
    fn lookups_delegate_to_the_runtime_view() {
        ...
        assert_eq!(handlers, [Path::new("inventory.west")]);
